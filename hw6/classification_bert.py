...
from transformers import T5Config, AutoModelForSequenceClassification
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, device, lr,trainplot,valplot):
...
def pre_process(model_name, batch_size, device, small_subset=False):
    ...
    max_len = 512 #128 before changed for t5
...
if __name__ == "__main__":
    ...
    for model in models:
        ...
        besttestacc=0
        for epoc in [5,7,9]:
            for learnr in [1e-4,5e-4,1e-3]:

                print(" >>>>>>>>  Starting training ... ")
                #train(...)
                train(pretrained_model, epoc, train_dataloader, validation_dataloader, args.device, learnr, args.train_plot, args.val_plot)
                # print the GPU memory usage just to make sure things are alright
                print_gpu_memory() 
                val_accuracy = evaluate_model(pretrained_model, validation_dataloader, args.device)
                print(f"Model is {model} learning rate{learnr} epoch is {epoc}")
                ...
                if tuple(val_accuracy.items())[0][1] > bestvalacc and tuple(test_accuracy.items())[0][1] > besttestacc:
                    bestvalacc = tuple(val_accuracy.items())[0][1]
                    besttestacc = tuple(test_accuracy.items())[0][1]
        model_acc[model] = [bestvalacc, besttestacc]
    ...
    modelsl = model_acc.keys()
    dev_acc = [model_acc[m][0] for m in modelsl]
    test_acc = [model_acc[m][1] for m in modelsl]
    x = range(len(modelsl))
    ax.bar(x, dev_acc, width=0.3, label='dev')
    ax.bar([i+0.4 for i in x], test_acc, width=0.3, label='test')
    ax.set_xticks(x)
    ax.set_xticklabels(modelsl,fontsize=8)
    ax.legend()
    ax.set_ylabel('Accuracy')
    ax.set_title('Dev and Test Accuracies for Different Models')
    plt.savefig(args.bar_plot)
    plt.show(block=True)"""
    # load the data and models
    pretrained_model, train_dataloader, validation_dataloader, test_dataloader = pre_process(args.model,
                                                                                             args.batch_size,
    ...
    for epoc in [5,7,9]:
        for learnr in [1e-4,5e-4,1e-3]:
            ...
            print_gpu_memory() 
            val_accuracy = evaluate_model(pretrained_model, validation_dataloader, args.device)
            print(f"Model is {args.model} learning rate{learnr} epoch is {epoc}")
            print(f" - Average DEV metrics: accuracy={val_accuracy}")
            test_accuracy = evaluate_model(pretrained_model, test_dataloader, args.device)
            print(f" - Average TEST metrics: accuracy={test_accuracy}")
            if tuple(val_accuracy.items())[0][1] > bestvalacc and tuple(test_accuracy.items())[0][1] > besttestacc:
                bestvalacc = tuple(val_accuracy.items())[0][1]
                besttestacc = tuple(test_accuracy.items())[0][1]
    model_acc[args.model] = [bestvalacc, besttestacc]
    print("If model gave out of memory before printing model acc keep that model as 0")
    print("model and accuracies ",model_acc)
...
"""
    ...
    for model in models:
        ...
        print(" >>>>>>>>  Starting training ... ")
            #train(...)
            train(pretrained_model, args.num_epochs, train_dataloader, validation_dataloader, args.device, args.lr, args.train_plot, args.val_plot)
