...
from transformers import get_scheduler, RagRetriever,RagConfig,RagModel
#os.environ["TOKENIZERS_PARALLELISM"] = "false"
from transformers import T5Config, AutoModelForSequenceClassification, RagTokenizer, RagTokenForGeneration, RagSequenceForGeneration,BartTokenizer
...
def evaluate_model(model, dataloader, tokenizer,device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            expected_answers.extend(["yes" if label.item() == 1 else "no" for label in labels])
...
def pre_process(model_name, batch_size, device,small_subset=False):
    ...
    #mytokenizer = AutoTokenizer.from_pretrained("facebook/dpr-question_encoder-single-nq-base")
    ...
    #mytokenizer = RagTokenizer(question_encoder_tokenizer, generator_tokenizer)
    mytokenizer = RagTokenizer.from_pretrained("facebook/rag-token-base")
    ...
    retriever = RagRetriever.from_pretrained("facebook/rag-token-base", index_name="exact", use_dummy_dataset=True,config = config)
    pretrained_model=RagModel.from_pretrained("facebook/rag-token-base", retriever=retriever,config = config)
    #pretrained_model = RagSequenceForGeneration.from_pretrained(model_name) # Changed model name here    
...
def main(experiment, small_subset, num_epochs, lr, batch_size, device, model, train_plot, val_plot, bar_plot):
    ...
    pretrained_model, train_dataloader, validation_dataloader, test_dataloader, tokenizer = pre_process(args.model,args.batch_size,args.device,args.small_subset)
